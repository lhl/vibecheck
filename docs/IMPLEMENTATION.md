...
- [ ] **Expose `title` in `SessionManager.discover()`** — read from Vibe meta.json `title` field (first 50 chars of initial user prompt, auto-generated by Vibe's `SessionLogger._get_title()`)
...
- [ ] **Tests** — `vibecheck/tests/test_sessions.py`
  - `GET /api/sessions` includes `title` field from meta.json
...
- [ ] **Session picker redesign** — replace raw UUID `<select>` dropdown with structured session list:
  - **Active = `controllable == true`** (TUI is attached/live, typically `attach_mode` in `{live, managed}`). An `observe_only` bridge created by WS auto-attach does NOT count as active.
  - **Active section** (default, always visible): active sessions sorted reverse-chron by `started_at`
  - **Each entry shows:** title (from `title` field — first 50 chars of initial prompt), relative age since `started_at` (e.g. "2h ago"), message count
  - **Title fallback:** if `title` is null/empty, display "New session" (also use when `message_count == 0`)
  - **Attention indicator:** sessions in `waiting_approval` or `waiting_input` state get a visual attention icon/badge so action-needed sessions stand out without resorting
  - **Auto-select:** latest active session selected on launch (currently picks `sessions[0]`)
  - **"Browse older sessions"** expandable/collapsible section for non-active sessions, same metadata display
...
- [x] L8: Camera/gallery → Mistral Large 3 multimodal → context for Vibe
  ...
  - **Completed implementation:** added `vibecheck/routes/vision.py` router, registered in `vibecheck/app.py`, and shipped coverage in `vibecheck/tests/test_vision.py`.
  - **Composer integration shipped:** `vibecheck/frontend/src/components/InputBar.svelte` now supports in-composer camera capture, processing/ready button states, hidden caption storage, and automatic `ATTACHED IMAGE:` block append at send.
  - **Verification completed:** backend tests (`uv run pytest vibecheck/tests/test_vision.py -v` and full suite) + frontend tests/build (`npm test -- src/components/InputBar.test.js`, `npm run build`) all passing.
...
**Deferred (post-hackathon):** See "Deferred" section in `docs/PLAN.md` for full details.
- Live demo mode (public read-only URL)
- QR code for audience participation
- Per-tool trust levels, time-boxed autonomy, risk scoring
- Replay mode, confetti
